...
public class HTExPresentationInfoManager : MonoBehaviour
{
    ...
    public void initExMemo()
    {
        ...
        PresentInfo.CSVTextBuffer = "対象音,正解の場所,回答した場所,正誤,回答時間,ヨー角,ピッチ角\n";
    }

    public void initExStepMemo(SOUNDTYPE targetSoundType,int targetSoundIndex, int answerSoundIndex, float responseTime, float yaw, float pitch)
    {
        // 実験のステップごとの処理

        // CSV保存バッファにメモ
        writeCSVTextBuffer(targetSoundType, targetSoundIndex, answerSoundIndex, responseTime, yaw, pitch);
        
        // 実験回数をメモ
        PresentInfo.exCount++;
    }

    public void writeCSVTextBuffer(SOUNDTYPE targetSoundType, int targetSoundIndex, int answerSoundIndex, float responseTime, float yaw, float pitch)
    {
        // CSV保存バッファにメモ
        ...
        // バッファに保存
        PresentInfo.CSVTextBuffer += $"{targetSoundType.ToString()},{targetSoundIndex},{answerSoundIndex},{isValid},{responseTime},{yaw},{pitch}\n";
    }
}
